...
const SceneThirtyone = (props) => {
  ...
  return (
    <section className="template-new-wrapper-scene1  scene-16 scene-31 slider-section">
      ...
      {/* {textShow ? <div class="char-counter"> {65 - content.length}  </div> : null} */}
      {textShow && content.length >= 55 ? <div class={content.length >= 75 ? "char-counter  red" : "char-counter "}>  {75 - content.length} </div> : null}
