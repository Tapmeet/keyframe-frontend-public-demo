...
const SceneThirtyEight = (props) => {
  ...
  return (
    <section className="template-new-wrapper-scene1 scene-2 scene-16 scene-38 section-18 scene-19 scene-21 scene-23 slider-section">
              ...
              {layout.map((single) => {
                return (
                  <textarea
                    ...
                    className={
                      "child-containers form-control border center  size-auto " +
      ...
      {/* {textShow ? <div class="char-counter"> {175 - content.length}  </div> : null} */}
      {textShow && content.length >= 155 ? <div class={content.length >= 175 ? "char-counter  red" : "char-counter "}>  {175 - content.length} </div> : null}
    </section>
  );
};
