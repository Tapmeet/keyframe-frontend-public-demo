...
const SceneSixteen = (props) => {
  ...
  return (
    <section className="template-new-wrapper-scene1  scene-16 slider-section">
      ...
      {/* {textShow ? <div class="char-counter"> {72 - content.length}  </div> : null} */}
      {textShow && content.length >= 52 ? <div class={content.length >= 72 ? "char-counter  red" : "char-counter "}>  {72 - content.length} </div> : null}
    </section>
  );
};
